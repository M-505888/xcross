...
void main() {
  ...
  test("copies the built target's Compose resources into the bundle", () async {
    ...
    // The bundle directory is the resources root: the resource path the runtime
    // asks for itself starts with `composeResources/`, so it appears twice.
    expect(
      File(
        p.join(
          appPath,
  ...
  test("prefers aggregated resources over the module's processed ones", () async {
    final fixture = _Fixture.create();
    final deviceFramework = fixture.frameworkPathFor('iosArm64');
    fixture.createInputsAt(deviceFramework);
    fixture.createResources(
  ...
  test('adds no compose-resources directory to a project without them', () async {
    final fixture = _Fixture.create()..createInputs();
    addTearDown(fixture.dispose);
    ...
    expect(Directory(p.join(appPath, 'compose-resources')).existsSync(), isFalse);
  });
}
